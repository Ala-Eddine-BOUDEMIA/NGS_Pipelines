...

{
  ...
  if (paired)
  {
    ...
    system(paste0("samtools view -@ 12 -b -f 128 -F 16 ",bamfile," > a.fwd1.bam"))


    # include reads that are mapped to the reverse strand (16) and
    # first in a pair (64): 64 + 16 = 80
    system(paste0("samtools view -@ 12 -b -f 80 ",bamfile," > a.fwd2.bam"))

    # combine the temporary files
    system(paste0("samtools merge -@ 12 -f ",fileOut,"_fwd.bam a.fwd1.bam a.fwd2.bam"))
    ...
    system(paste0("samtools view -@ 12 -b -f 144 ",bamfile," > a.rev1.bam"))

    # include reads that are first in a pair (64), but
    # exclude those ones that map to the reverse strand (16)
    system(paste0("samtools view -@ 12 -b -f 64 -F 16 ",bamfile," > a.rev2.bam"))


    # merge the temporary files
    system(paste0("samtools merge -@ 12 -f ",fileOut,"_rev.bam a.rev1.bam a.rev2.bam"))
  ...
  else
  {
    ...
    print("Seperating the forward strand bam.")
    # Forward strand.
    system(paste0("samtools view -@ 12 -bh -f 16 ",bamfile," > ",fileOut,"_fwd.bam"))
    system(paste0("samtools index ",fileOut,"_fwd.bam"))

    print("Seperating the reverse strand bam.")
    # Reverse strand
    system(paste0("samtools view -@ 12 -bh -F 16 ",bamfile," > ",fileOut,"_rev.bam"))
    system(paste0("samtools index ",fileOut,"_rev.bam"))
  ...
  for (i in c(1:length(chrom))){
    ...
    system(paste0("samtools view -@ 12 ",fileOut,"_fwd.bam ",chr.name," > fwd_",chr.name,".sam"))
    ...
    system(paste0("samtools view -@ 12 ",fileOut,"_rev.bam ",chr.name," > rev_",chr.name,".sam"))
...
parser <- ArgumentParser(description = 'OKseqHMM')

parser$add_argument('--bamfile', '-b', help = 'BAM file path')
parser$add_argument('--chrsizes', '-c', help = 'Chromosomes sizes file path')
parser$add_argument('--fileOut', '-f', help = 'Output directory')
parser$add_argument('--thresh', '-th', help = 'Threshold', type = 'integer')
parser$add_argument('--winS', '-w', help = 'Window size in Kb', type = 'integer')
parser$add_argument('--binSize', '-bs', help = 'Bin size', type = 'integer')

xargs<- parser$parse_args()

bamfile <- xargs$bamfile
chrsizes <- xargs$chrsizes
fileOut <- xargs$fileOut
thresh <- xargs$thresh
winS <- xargs$winS
binSize <- xargs$binSize

OKseqHMM(bamfile,chrsizes,fileOut, thresh, winS, binSize)
