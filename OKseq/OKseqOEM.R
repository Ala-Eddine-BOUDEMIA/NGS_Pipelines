...
OKseqOEM <- function(bamInF, bamInR, chrsizes, fileOut, binSize, binList)
{
  ...
  for (i in c(1:nrow(chr.sizes))){
    ...
    if (paired)
    {
      ...
      system(paste0("samtools view -@ 12 -q 1 -f 0x42 -F 0x4 ",bamInF," ",chr.name," > fwd_",chr.name,".sam"))
      ...
      system(paste0("samtools view -@ 12 -q 1 -f 0x42 -F 0x4 ",bamInR," ",chr.name," > rev_",chr.name,".sam"))
      ...
      system(paste0("samtools view -@ 12 ",bamInF," ",chr.name," > fwd_",chr.name,".sam"))
      system(paste0("awk '$3~/^", chr.name, "$/ {print $2 \"\t\" $4}' fwd_",chr.name,".sam > fwd_",chr.name,".txt"))

      print(paste0("Calculating ",binSize,"bp binsize coverage for reverse strand."))
      system(paste0("samtools view -@ 12 ",bamInR," ",chr.name," > rev_",chr.name,".sam"))
      system(paste0("awk '$3~/^", chr.name, "$/ {print $2 \"\t\" $4}' rev_",chr.name,".sam > rev_",chr.name,".txt"))
    }
...
parser <- ArgumentParser(description = 'OKseqOEM')
parser$add_argument('--bamInF', '-bIf', help = 'BAM forward strands file path')
parser$add_argument('--bamInR', '-bIr', help = 'BAM reverse strands file path')
parser$add_argument('--chrsizes', '-c', help = 'Chromosomes sizes file path')
parser$add_argument('--fileOut', '-f', help = 'Output directory')
parser$add_argument('--binSize', '-bs', help = 'Bin size', type = 'double')
parser$add_argument('--binList', '-bl', help = 'Bin list')
...
binList <- xargs$binList
binList <- as.double(as.list(strsplit(binList, ",")[[1]]))

OKseqOEM(bamInF, bamInR, chrsizes, fileOut, binSize, binList)
